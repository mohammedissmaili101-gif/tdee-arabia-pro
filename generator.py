...
def format_content(text):
    # تم تعديل RegEx للسماح بالحروف العربية والإنجليزية والأرقام لضمان عدم مسح النص
    # حذفنا السطر الذي كان يمسح كل شيء غير العربي
...
def get_pixabay_image(keyword):
    ...
    return "https://images.unsplash.com/photo-1517836357463-d25dfeac3438?q=80&w=1200&h=800&auto=format&fit=crop"
...
def update_blog_list(file_slug, title, image_url, category):
    ...
    if not os.path.exists(blog_file):
        # إنشاء ملف blog.html إذا لم يكن موجوداً مع الماركر
        initial_html = f'''<!DOCTYPE html><html lang="ar" dir="rtl"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><script src="https://cdn.tailwindcss.com"></script><link href="https://fonts.googleapis.com/css2?family=Cairo:wght@400;700;900&display=swap" rel="stylesheet"><style>body{{font-family:"Cairo", sans-serif;}} .blog-card{{transition: transform 0.3s ease;}} .blog-card:hover{{transform: translateY(-8px);}}</style></head><body class="bg-slate-50"><nav class="bg-white/80 backdrop-blur-md p-8 shadow-sm border-b sticky top-0 z-50"><div class="max-w-7xl mx-auto flex justify-between items-center"><h1 class="text-3xl font-black tracking-tighter text-slate-900">TDEE <span class="text-blue-600 italic">ARABIA</span></h1><input type="text" id="searchInput" onkeyup="searchPosts()" placeholder="إبحث في المجلة..." class="w-72 px-6 py-4 rounded-[2rem] border-2 border-slate-100 outline-none focus:border-blue-600 transition-all text-right shadow-inner"></div></nav><main class="max-w-7xl mx-auto px-8 py-20"><div id="blog-grid" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12 text-right">{marker}\n{new_card}</div></main><script>function searchPosts() {{ let input = document.getElementById("searchInput").value.toLowerCase(); let cards = document.querySelectorAll(".blog-card"); cards.forEach(card => {{ let title = card.getAttribute("data-title").toLowerCase(); card.style.display = title.includes(input) ? "block" : "none"; }}); }}</script></body></html>'''
...
def generate_post():
    ...
    try:
        # تحسين البرومبت لإجبار النموذج على عدم كتابة أي مقدمات خارج المقال
        prompt = (f"اكتب مقالاً احترافياً في قسم '{cat}'. "
                  f"القاعدة الأولى: أول سطر يجب أن يكون هو 'العنوان' فقط بدون أي رموز مثل # أو **. "
                  f"القاعدة الثانية: ابدأ المقال فوراً من السطر الثاني. "
                  f"القاعدة الثالثة: استخدم **للعناوين الفرعية** و * للنقاط. "
                  f"اجعل الأسلوب بشرياً وعميقاً جداً.")

        response = client.chat.completions.create(
            messages=[
                {"role": "system", "content": "أنت خبير كمال أجسام وتغذية وتتحدث العربية بطلاقة وبأسلوب مشوق."},
