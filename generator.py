...
def update_blog_list(file_slug, title, image_url, category):
    ...
    if not os.path.exists(blog_file):
        initial_html = f'''<!DOCTYPE html><html lang="ar" dir="rtl"><head><meta charset="UTF-8"><script src="https://cdn.tailwindcss.com"></script><link href="https://fonts.googleapis.com/css2?family=Cairo:wght@400;700;900&display=swap" rel="stylesheet"><style>body{{font-family:"Cairo", sans-serif;}}</style></head><body class="bg-slate-50"><nav class="bg-white p-6 shadow-sm border-b sticky top-0 z-50"><div class="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-4"><h1 class="text-2xl font-black text-blue-600">TDEE ARABIA 🔥</h1><input type="text" id="searchInput" onkeyup="searchPosts()" placeholder="إبحث عن مقال..." class="w-full md:w-96 px-5 py-3 rounded-2xl border border-slate-200 outline-none text-right"></div></nav><main class="max-w-7xl mx-auto px-6 py-12"><div id="blog-grid" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">{marker}{new_card}</div></main><script>function searchPosts() {{ let input = document.getElementById("searchInput").value.toLowerCase(); let cards = document.querySelectorAll(".blog-card"); cards.forEach(card => {{ let title = card.getAttribute("data-title").toLowerCase(); card.style.display = title.includes(input) ? "block" : "none"; }}); }}</script></body></html>'''
...
def generate_post():
    sport_keywords = ["gym", "workout", "fitness", "bodybuilding", "protein"]
    topics = {"تنشيف": "أسرار التنشيف العضلي", "تضخيم": "دليل التضخيم الشامل", "تغذية": "أفضل وجبات الرياضيين"}
    cat = random.choice(list(topics.keys()))
    title = topics[cat] + f" لعام {random.randint(2025, 2026)}"
    
    try:
        response = client.chat.completions.create(
            messages=[{"role": "user", "content": f"اكتب مقال SEO رياضي مطول جدا بالعربية عن {title}. استعمل العناوين."}],
        ...
        full_html = f'''<!DOCTYPE html><html lang="ar" dir="rtl"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><script src="https://cdn.tailwindcss.com"></script><link href="https://fonts.googleapis.com/css2?family=Cairo:wght@400;700;900&display=swap" rel="stylesheet"><style>body{{font-family:"Cairo", sans-serif;}}</style></head><body class="bg-slate-50"><article class="max-w-4xl mx-auto py-16 px-6 bg-white min-h-screen shadow-xl rounded-b-[3rem]"><a href="./blog.html" class="text-blue-600 font-bold mb-8 inline-block hover:underline">← العودة للمدونة</a><img src="{img}" class="w-full h-96 object-cover rounded-3xl mb-12 shadow-md border-4 border-white"><h1 class="text-4xl md:text-5xl font-black mb-10 text-slate-900 leading-tight border-b-4 border-blue-600 pb-4">{title}</h1><div class="content">{body}</div></article></body></html>'''
