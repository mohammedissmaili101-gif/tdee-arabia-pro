...
# مصفوفة الكلمات الدلالية للصور باش نوعو المحتوى البصري
gym_keywords = ["gym", "fitness", "bodybuilding", "workout", "supplements", "healthy-food", "crossfit"]
...
def update_blog_list(file_slug, title, image_url, category):
    ...
    <div class="blog-card bg-white rounded-[2.5rem] shadow-xl overflow-hidden hover:scale-105 transition-all duration-500 border border-slate-100 group" data-category="{category}">
        <div class="relative overflow-hidden h-64">
            <img src="{image_url}" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" onerror="this.src='https://images.unsplash.com/photo-1534438327276-14e5300c3a48?auto=format&fit=crop&w=800'">
...
def generate():
    ...
    # حل مشكلة تكرار الصور: جلب صورة عشوائية من Unsplash بناءً على كلمة رياضية + رقم عشوائي لضمان التغيير
    keyword = random.choice(gym_keywords)
    image = f"https://source.unsplash.com/featured/1200x800?{keyword}&sig={random.randint(1, 1000)}"
    ...
    try:
        ...
        <main class="max-w-4xl mx-auto my-10 px-4"><div class="relative h-[450px] rounded-[3rem] overflow-hidden shadow-2xl mb-[-80px] z-10 border-8 border-white"><img src="{image}" class="w-full h-full object-cover" onerror="this.src='https://images.unsplash.com/photo-1517836357463-d25dfeac3438?auto=format&fit=crop&w=1200'"><div class="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent"></div><div class="absolute bottom-12 right-12 text-white"><span class="bg-blue-600 px-4 py-1 rounded-full text-sm font-bold mb-4 inline-block">{category}</span><h1 class="text-4xl md:text-6xl font-black leading-tight drop-shadow-2xl">{title}</h1></div></div>
