...
def generate():
    # تنويع المواضيع
    topics = ["تضخيم العضلات الصافي", "تنشيف الدهون العنيدة", "أهم المكملات الغذائية", "تمارين الكارديو لحرق السعرات", "أخطاء التغذية الشائعة"]
    selected_topic = random.choice(topics)
    weight = random.randint(60, 100)
    image = random.choice(gym_images)
    title = f"{selected_topic} لمن وزنه {weight} كجم"
    
    try:
        res = client.chat.completions.create(
            messages=[{"role": "user", "content": f"اكتب مقال SEO طويل واحترافي بالعربية عن {title}. استخدم لغة حماسية وعناوين فرعية."}],
        ...
        article_html = f"""
        <!DOCTYPE html><html lang="ar" dir="rtl"><head><meta charset="UTF-8"><title>{title}</title><script src="https://cdn.tailwindcss.com"></script><link href="https://fonts.googleapis.com/css2?family=Cairo:wght@400;700;900&display=swap" rel="stylesheet"></head>
        <body class="bg-slate-50 font-['Cairo'] text-right"><nav class="bg-white p-4 border-b sticky top-0 z-50"><div class="max-w-4xl mx-auto"><a href="/blog.html" class="text-blue-600 font-bold">← العودة للمدونة</a></div></nav>
        <main class="max-w-4xl mx-auto my-12 px-4"><img src="{image}" class="w-full h-96 object-cover rounded-[2.5rem] shadow-2xl mb-12"><article class="bg-white p-10 rounded-[2.5rem] shadow-sm">
        <h1 class="text-4xl md:text-5xl font-black mb-8 text-slate-900 leading-tight">{title}</h1><div class="text-xl leading-relaxed text-slate-700 space-y-6">{body}</div></article></main></body></html>
        """
