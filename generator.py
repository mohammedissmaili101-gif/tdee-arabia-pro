...
def get_pixabay_image(keyword):
    try:
        ...
        # إضافة User-Agent لضمان عدم الحظر من Pixabay
...
def generate_post():
    ...
    try:
        # تحسين البرومبت لضمان استلام محتوى كامل
        prompt = (f"اكتب مقالاً طويلاً ومفصلاً في قسم '{cat}'. "
                  f"أول سطر: العنوان فقط. "
                  f"من السطر الثاني: ابدأ المقال فوراً. "
                  f"استخدم **للعناوين الفرعية** و * للنقاط. "
                  f"أريد 500 كلمة على الأقل بأسلوب بشري.")

        response = client.chat.completions.create(
            messages=[
                {"role": "system", "content": "أنت خبير كمال أجسام وتغذية عربي محترف."},
        ...
        lines = [l for l in full_text.split('\n') if l.strip()] # إزالة الأسطر الفارغة في البداية
