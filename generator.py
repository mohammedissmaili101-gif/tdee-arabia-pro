...
# 2. تنسيقات الفقرات الجديدة (الألوان المريحة للعين)
paragraph_styles = [
    "bg-blue-50 border-blue-200 text-blue-900",     # أزرق فاتح
    "bg-slate-50 border-slate-200 text-slate-900",   # رمادي هادئ
    "bg-indigo-50 border-indigo-200 text-indigo-900", # بنفسجي فاتح
    "bg-emerald-50 border-emerald-200 text-emerald-900" # أخضر زمردي
]
...
def get_gym_image():
    # استعملت loremflickr للحصول على صور رياضية عشوائية جديدة
...
def update_blog_list(file_slug, title, image_url, category):
    ...
    # تنسيق البطاقة في صفحة المدونة (Blog Card)
    new_card = f"""
    ...
    if not os.path.exists(blog_file):
        # إنشاء صفحة المدونة الأساسية بتنسيق نظيف
...
def generate_post():
    ...
    try:
        ...
        # تنسيق صفحة المقال الكاملة (تلوين الفقرات واحترافية التصميم)
...
if __name__ == "__main__":
    generate_post()
```ملي شفت هاد التصاور، عرفت باللي الـ Marker ديالنا خدام، ولكن التنسيق ديال الألوان طلع كارثي.

دابا، صلحت ليك المشكل نهائياً. في بلاصة ما نخدمو بالأحمر والأزرق الفاقع، درت تنسيقات ألوان جديدة (الأزرق الفاتح، الرمادي، البنفسجي، والأخضر الزمردي الهادئ). الكود دابا كيختار لون مختلف لكل فقرة باش المقال يجي منظم ومريح للعين، مع الحفاظ على العنوان الرياضي والزر الأزرق.

ها هو الكود المصحح فـ `generator.py` (حطو فبلاصت القديم):
